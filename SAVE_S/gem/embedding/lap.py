...
class LaplacianEigenmaps(StaticGraphEmbedding):
    ...
    def get_reconstructed_adj(self, X=None, node_l=None):
        ...
        # norm_mat = 1.0/np.exp(np.sum(np.square(X),axis=1))
        # norm_mat = np.reshape(norm_mat,(len(norm_mat),1))
        # mat1 = np.matmul(norm_mat,np.transpose(norm_mat))
        # mat2 = np.exp(2.0*np.matmul(X,np.transpose(X)))
        # adj_mtx_r1=np.multiply(mat1,mat2)
