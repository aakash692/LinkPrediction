...
if not os.path.exists('SAVER_TE'):
	os.makedirs('SAVER_TE')
...
for grp in xrange(len(list_graphs)):
	for x in xrange(num_samples):
		...
		if not os.path.exists('SAVER_TE/'+fig_name[grp]):
			os.makedirs('SAVER_TE/'+fig_name[grp])
		...
		file_name='SAVER_TE/'+fig_name[grp]+'/graph_and_samples.p'
		...
		for dim in dimensions:
			...
			file_name='SAVER_TE/'+fig_name[grp]+'/DEEPWALK_'+str(dim)
			parameter_file=open(file_name, 'wb')
		...
		for dim in dimensions:
			...
			X, _ = embedding.learn_embedding(graph=train_digraph, no_python=False)
			file_name='SAVER_TE/'+fig_name[grp]+'/n2vA_'+str(dim)
			parameter_file=open(file_name, 'wb')
			pickle.dump(X,parameter_file)
			parameter_file.close()
		...
		for dim in dimensions:
			...
			X, _ = embedding.learn_embedding(graph=train_digraph, no_python=False)
			file_name='SAVER_TE/'+fig_name[grp]+'/n2vB_'+str(dim)
			parameter_file=open(file_name, 'wb')
			pickle.dump(X,parameter_file)
			parameter_file.close()
		...
		for dim in dimensions:
			...
			X, _ = embedding.learn_embedding(graph=train_digraph, no_python=False)
			file_name='SAVER_TE/'+fig_name[grp]+'/VERSE_'+str(dim)
			parameter_file=open(file_name, 'wb')
			pickle.dump(X,parameter_file)
			parameter_file.close()				
